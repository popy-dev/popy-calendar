...
class NativeFormatPregMatch implements SymbolParserInterface
{
    ...
    public function parseSymbol(FormatToken $token, FormatParserInterface $formater)
    {
        if ($token->is('y')) {
            ...
            return new SimplePreg($token, '\d\d');
        }
        ...
        if ($token->isOne('Y', 'o')) {
            ...
            return new SimplePreg($token, '\d\d\d\d');
        }

        if ($token->is('L')) {
            // L   Whether it's a leap year
            return new SimplePreg($token, '[01]');
        }

        if ($token->is('F')) {
            // F   A full textual representation of a month, such as January or March
            return new SimplePreg($token, '\S.*?');
        }

        if ($token->is('M')) {
            // M   A short textual representation of a month, three letters    Jan through Dec
            return new SimplePreg($token, '.{1,3}');
        }
        ...
        if ($token->is('S')) {
            ...
            return new SimplePreg($token, '\S{1,2}');
        }

        if ($token->is('w')) {
            // w   Numeric representation of the day of the week   0 (for Sunday) through 6 (for Saturday)
            return new SimplePreg($token, '\d');
        }

        if ($token->is('z')) {
            // z   The day of the year (starting from 0)
            return new SimplePreg($token, '\d{1,3}');
        }
        ...
        if ($token->isOne('a', 'A')) {
            ...
            return new SimplePreg($token, '[apAP][mM]');
        }

        if ($token->is('B')) {
            // B   Swatch Internet time    000 through 999
            return new SimplePreg($token, '\d\d\d');
        }
        ...
        if ($token->is('u')) {
            // u   Microseconds
            return new SimplePreg($token, '\d{6}');
        }
        ...
        if ($token->is('O')) {
            ...
            return new SimplePreg($token, '[+\-]\d{4}');
        }
        ...
        if ($token->is('T')) {
            ...
            return new SimplePreg($token, '[A-Z]{1,3}');
        }

        if ($token->is('Z')) {
            // Z   Timezone offset in seconds. The offset for timezones west of UTC is always negative, and for those east of UTC is always positive.  -43200 through 50400
            return new SimplePreg($token, '-?\d{1,5}');
        }
